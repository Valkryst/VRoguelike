...
# Display Repo:
echo -e "Repo Slug:"
echo -e "\tExpected:\tValkryst/VRoguelike"
...
if [ "$TRAVIS_REPO_SLUG" == "Valkryst/VRoguelike" ] &&
   ...
   [ "$TRAVIS_BRANCH" == "master" ]; then
    ...
    cd $HOME
    git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/Valkryst/VRoguelike gh-pages > /dev/null
    
    cd gh-pages
    git rm -r *
    git config user.name "Valkryst"
    git config user.email "valkryst@valkryst.com"
    cp -r /home/travis/build/Valkryst/VRoguelike/target/site/apidocs/* .
    git add .
